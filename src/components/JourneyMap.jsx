...
import { Float, Stars, Cloud, Sparkles, RoundedBox, Environment as DreiEnvironment } from '@react-three/drei';
        ...
        <group ref={busRef} position={[0, 0.6, 0]}>
            ...
            <group position={[0, 1.0, 6.4]} rotation={[Math.PI / 12, 0, 0]}>
                <mesh position={[-1.2, 0, 0]}>
                    ...
                    <meshStandardMaterial color="#fff" emissive="#fff" emissiveIntensity={10} toneMapped={false} />
                </mesh>
                <mesh position={[1.2, 0, 0]}>
                    <boxGeometry args={[0.6, 0.1, 0.1]} />
                    <meshStandardMaterial color="#fff" emissive="#fff" emissiveIntensity={10} toneMapped={false} />
    ...
    return (
        <group>
            ...
            {Array.from({ length: 300 }).map((_, i) => (
                <mesh key={i} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, -i * 10 + 20]}>
    ...
    return (
        <>
            ...
            {Array.from({ length: 100 }).map((_, i) => (
                <group key={i} position={[i % 2 === 0 ? -18 : 18, 12, -i * 25]}>
                    <mesh position={[0, -6, 0]}>
                        <cylinderGeometry args={[0.2, 0.2, 12]} />
                        <meshStandardMaterial color="#111" />
                    </mesh>
                    <pointLight intensity={20} distance={40} color="#ffaa00" decay={2} />
                    <mesh position={[0, 0, 0]}>
...
const JourneyMap = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            // We want 0 when the section top hits the top of viewport (or slightly before?)
            // The section is 1200vh.
            // We want the journey to start when the section becomes sticky.

            // The sticky container is top-0. So it becomes sticky when rect.top <= 0.
            // It stops being sticky when rect.bottom <= viewportHeight.
    ...
    return (
        <section id="journey" className="h-[1200vh] relative bg-dark-base">
